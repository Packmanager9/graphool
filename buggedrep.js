// window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Node {
        ...
        tupe(macro, me, node){
            ...
            //  //console.log(babble)
            for(let t= 0 ;t<babble.length;t++){
            let offset = table[babble[t]]
        ...
        energyBalance(){
            ...
            for(let t =0 ;t<glist.length;t++){
                for(let k = 0;k<glist[t].link.list.length;k++){
                      if(f.includes((glist[t].link.list[k])) || glist.length > globalnodes.length){
                      }else{
                        f.push((glist[t].link.list[k]))
            ...
            if(g >= (100/Math.sqrt(Math.sqrt(m))) && glist.length>1 && macrodupe == 0){
                ...
                for(let t =0 ;t<nodds.length;t++){
                    nodds[t].tupe(macro, nodds[t].t, nodds[t])
        ...
        connect(node) {
            ...
            if(this.neighbors.includes(node.id) || node.neighbors.includes(this.id)){
                return
            }
